...
const {log, LogLevel, asyncForEach} = require(path.resolve(__dirname, 'helper.master.js'));
...
const fullTestForMasterStrategies = require(path.resolve(__dirname, `ch.mobiliar.${projectId}`));
...
execute('idevice_id -l').then(async (attachedUdids) => {
    ...
    asyncForEach(fullTestForMasterStrategies.strategies, (strategy, strategyIndex) => {
        if (!strategy.enabled) {
            return true;
        }

        const {debug, mode} = strategy;
        log(LogLevel.Log, `Starting Automation Strategy for: ${fullTestForMasterStrategies.bundleId}`);
        ...
        let params;
        let signingIdentity = {
            xcodeOrgId: fullTestForMasterStrategies.xcodeOrgId,
            xcodeSigningId: fullTestForMasterStrategies.xcodeSigningId,
            bundleId: fullTestForMasterStrategies.bundleId,
            appPath: fullTestForMasterStrategies.appPath,
            simulatorAppPath: fullTestForMasterStrategies.simulatorAppPath
        };
        ...
        switch (mode) {
            case 'SINGLE':
                ...
                params = ['./config/strategies.master/wdio.conf.master.js', '--deviceInfo', configFilePath];
                singleDevice = {
                    ...singleDevice,
                    ...strategy,
                    ...signingIdentity
        ...
        forEach(strategy.suites, (suite, suiteName) => {
            ...
            suites[suiteName] = map(suite.testCases, (testCase) =>
                path.join(
                    process.cwd(),
                    '/dist/apps/',
                    fullTestForMasterStrategies.bundleId,
                    'specs',
